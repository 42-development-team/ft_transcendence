...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleFriendRequest(userId: number, friendId: number) {
        const userSocket = this.clients.find(async c => c.id === await this.userService.getUserSocketFromId(userId));
        if (userSocket) {
            userSocket.emit('friendRequest', { friendId });
        }
        const friendSocket = this.clients.find(async c => c.id === await this.userService.getUserSocketFromId(Number(friendId)));
        if (friendSocket) {
