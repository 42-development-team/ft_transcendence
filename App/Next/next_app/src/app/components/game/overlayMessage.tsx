...
const OverlayMessage = ({ message, surrender, id, userId } : { message: any, surrender: (id: number, userId: number) => void, id: number, userId: number}) => {
  ...
  return (
    <div className={`fixed inset-0 flex flex-col items-center justify-center w-[auto] h-[auto] z-1 ${textColor} bg-base bg-opacity-20 backdrop-blur-xl`}>
      <div className="flex text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-center">{message}</div>
        <div className='flex'>
            <CustomBtn onClick={() => surrender(0, 0)} color='bg-red' disable={false} >Surrender</CustomBtn>
        </div>
    </div>
  );
}
